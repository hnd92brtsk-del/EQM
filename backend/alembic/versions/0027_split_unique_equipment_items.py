"""split unique equipment items

Revision ID: 0027_split_unique_equipment_items
Revises: 0026_create_pid_process
...
revision = "0027_split_unique_equipment_items"
down_revision = "0026_create_pid_process"
branch_labels = None
depends_on = None
...
def _split_cabinet_items(connection, unique_equipment):
    rows = connection.execute(
        sa.select(cabinet_items).where(
            ...
            cabinet_items.c.equipment_type_id.in_(unique_equipment.keys()),
        )
    ).mappings().all()
    if not rows:
        return
    ...
    for row in rows:
        ...
        missing_signals_on_original = (
            equipment_row["is_channel_forming"] and signal_counts.get(row["id"], 0) == 0
        )
        if missing_signals_on_original:
        ...
        for _ in range(row["quantity"] - 1):
            ...
            new_item_id = connection.execute(insert_stmt).scalar_one()
...
def _split_assembly_items(connection, unique_equipment):
    rows = connection.execute(
        sa.select(assembly_items).where(
            ...
            assembly_items.c.equipment_type_id.in_(unique_equipment.keys()),
        )
    ).mappings().all()
    if not rows:
        return
    ...
    for row in rows:
        ...
        if payload:
            connection.execute(sa.insert(assembly_items), payload)


def upgrade():
    op.drop_constraint("uq_cabinet_items_cb_eqtype", "cabinet_items", type_="unique")
    op.drop_constraint("uq_assembly_items_as_eqtype", "assembly_items", type_="unique")
...
def downgrade():
    op.create_unique_constraint(
        "uq_assembly_items_as_eqtype",
        "assembly_items",
        ["assembly_id", "equipment_type_id"],
    )
    op.create_unique_constraint(
        "uq_cabinet_items_cb_eqtype",
        "cabinet_items",
        ["cabinet_id", "equipment_type_id"],
    )
