...
export function DropdownMenuContent({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  ...
  if (!open) return null;
  return <div className={cn("absolute right-0 top-[calc(100%+6px)] z-50 min-w-[220px] rounded-none border border-slate-200 bg-white p-1 shadow-lg", className)} {...props} />;
}
...
export function DropdownMenuItem({
  ...
  return (
    <button
      type="button"
      className={cn(
        "flex w-full items-center gap-2 rounded-none px-2 py-2 text-sm text-slate-700 transition hover:bg-slate-100 disabled:pointer-events-none disabled:opacity-50",
