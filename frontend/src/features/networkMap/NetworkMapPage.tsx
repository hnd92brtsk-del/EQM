...
export default function NetworkMapPage() {
  ...
  const renderDiagnosticsPanel = () => (
    <div className="space-y-3">
      {validation.length === 0 ? <div className="border border-emerald-300/75 bg-[color-mix(in_srgb,#14532d_16%,var(--eqm-ui-panel)_84%)] p-4 text-sm text-emerald-100">{t("pages.networkMap.validation.clear")}</div> : null}
      {validation.map((item) => <div key={item.id} className={cn("border p-3 shadow-sm", item.severity === "critical" ? "border-red-300/80 bg-[color-mix(in_srgb,#7f1d1d_18%,var(--eqm-ui-panel)_82%)]" : "border-amber-300/80 bg-[color-mix(in_srgb,#78350f_16%,var(--eqm-ui-panel)_84%)]")}><div className={cn("text-sm font-semibold", item.severity === "critical" ? "text-red-100" : "text-amber-100")}>{item.title}</div><div className={cn("mt-1 text-xs", item.severity === "critical" ? "text-red-200/85" : "text-amber-100/80")}>{item.detail}</div></div>)}
    </div>
  );
  ...
  return (
    <div className="eqm-canvas-page space-y-4 text-slate-900">
        ...
        <Card className="overflow-hidden border-slate-200 shadow-none">
            ...
            <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div ref={addNodeMenuRef} className="relative">
                  ...
                  {isAddNodeMenuOpen ? <div className="absolute left-0 top-full z-20 mt-2 max-h-72 w-48 overflow-auto border border-[var(--eqm-ui-border-strong)] bg-[var(--eqm-ui-panel)] p-1 shadow-xl">{NETWORK_NODE_TYPES.map((type) => <button key={type} type="button" className="flex w-full items-center justify-between px-3 py-2 text-left text-sm text-[var(--eqm-ui-text)] transition hover:bg-[var(--eqm-ui-panel-alt)]" onClick={() => { addManualNode(type); setIsAddNodeMenuOpen(false); }}>{getNodeTypeLabel(type)}</button>)}</div> : null}
                </div>
              ...
              <div className="flex flex-col gap-3 xl:items-end">
                <div ref={searchPanelRef} className="relative min-w-0 xl:w-[280px]">
                  ...
                  {canvasSearch.trim() && isSearchResultsOpen ? <div className="absolute left-0 top-full z-20 mt-2 max-h-72 w-full overflow-auto border border-[var(--eqm-ui-border-strong)] bg-[var(--eqm-ui-panel)] shadow-xl">{searchResults.length ? searchResults.map((node) => <button key={node.id} type="button" className="block w-full border-b border-[var(--eqm-ui-border)] px-3 py-2 text-left transition hover:bg-[var(--eqm-ui-panel-alt)] last:border-b-0" onClick={() => focusSearchResult(node.id)}><div className="truncate text-sm font-semibold text-[var(--eqm-ui-text)]">{node.name}</div><div className="mt-1 text-xs text-[var(--eqm-ui-muted)]">{getNodeTypeLabel(node.type)} · {node.ip || "IP не задан"}{node.vlan ? ` · VLAN ${node.vlan}` : ""}</div></button>) : <div className="px-3 py-3 text-sm text-[var(--eqm-ui-muted)]">Совпадений не найдено.</div>}</div> : null}
                </div>
                <div className="flex flex-wrap items-center gap-2">
          ...
          <CardContent className="p-0">
            <div
              ...
              className={cn("eqm-canvas-shell relative h-[calc(100vh-260px)] min-h-[680px] overflow-hidden", isFullscreen && "h-screen min-h-screen")}
            >
              ...
              {showMiniMap ? <Card className="absolute bottom-4 right-4 w-[190px] rounded-none border-[var(--eqm-ui-border-strong)] bg-[color-mix(in_srgb,var(--eqm-ui-panel)_94%,transparent)] shadow-[0_18px_36px_rgba(15,23,42,0.18)] backdrop-blur"><CardContent className="p-3"><div className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--eqm-ui-muted)]">Миникарта</div><svg width="100%" height="120" viewBox={`${miniMapViewBox.x} ${miniMapViewBox.y} ${miniMapViewBox.width} ${miniMapViewBox.height}`} onPointerDown={handleMiniMapPointerDown} style={{ cursor: "pointer" }}>{topology.edges.map((edge) => <path key={edge.id} d={edgePath(edge)} fill="none" stroke="color-mix(in srgb, var(--eqm-ui-muted) 78%, transparent)" strokeWidth="8" strokeLinecap="round" />)}{topology.nodes.map((node) => <rect key={node.id} x={node.x} y={node.y} width={nodeWidth} height={nodeHeight} fill={selectedNodeIds.includes(node.id) ? "#60a5fa" : "color-mix(in srgb, var(--eqm-ui-panel-alt) 88%, #cbd5e1 12%)"} stroke={selectedNodeIds.includes(node.id) ? "#bfdbfe" : "color-mix(in srgb, var(--eqm-ui-muted) 75%, transparent)"} strokeWidth="3" onPointerDown={(event) => { event.stopPropagation(); focusNode(node.id); }} style={{ cursor: "pointer" }} />)}{miniMapViewportRect ? <rect x={miniMapViewportRect.x} y={miniMapViewportRect.y} width={miniMapViewportRect.width} height={miniMapViewportRect.height} fill="rgba(96,165,250,0.12)" stroke="#60a5fa" strokeWidth="4" pointerEvents="none" /> : null}</svg></CardContent></Card> : null}
              <div className="pointer-events-none absolute bottom-4 left-4 border border-[var(--eqm-ui-border-strong)] bg-[color-mix(in_srgb,var(--eqm-ui-panel)_92%,transparent)] px-3 py-2 text-[11px] text-[var(--eqm-ui-muted)] shadow-[0_14px_32px_rgba(15,23,42,0.14)] backdrop-blur"><div className="flex items-center gap-2"><Grip className="h-3.5 w-3.5" /> Панорама: drag по пустому холсту или wheel</div><div className="mt-1 flex items-center gap-2"><ArrowDownUp className="h-3.5 w-3.5" /> Выбор: клик, Shift-мультивыбор или рамка</div><div className="mt-1 flex items-center gap-2"><Route className="h-3.5 w-3.5" /> Маршрут и диагностика управляются справа</div></div>
            </div>
          </CardContent>
        </Card>
      ...
      {isSidebarDrawerOpen ? <div className="fixed inset-0 z-50 bg-slate-900/35 xl:hidden" onClick={() => setIsSidebarDrawerOpen(false)}><div className="eqm-canvas-panel h-full w-[min(92vw,360px)] p-4" onClick={(event) => event.stopPropagation()}><div className="mb-3 flex items-center justify-between"><div className="text-sm font-semibold text-[var(--eqm-ui-text)]">Контуры</div><Button size="sm" variant="outline" onClick={() => setIsSidebarDrawerOpen(false)}>Закрыть</Button></div>{sidebarPanel}</div></div> : null}
      {isInspectorDrawerOpen ? <div className="fixed inset-0 z-50 bg-slate-900/35 xl:hidden" onClick={() => setIsInspectorDrawerOpen(false)}><div className="eqm-canvas-panel ml-auto h-full w-[min(94vw,420px)] p-4" onClick={(event) => event.stopPropagation()}><div className="mb-3 flex items-center justify-between"><div className="text-sm font-semibold text-[var(--eqm-ui-text)]">Свойства</div><Button size="sm" variant="outline" onClick={() => setIsInspectorDrawerOpen(false)}>Закрыть</Button></div>{inspectorPanel}</div></div> : null}
    </div>
  );
}
