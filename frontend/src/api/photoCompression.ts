export const PHOTO_SERVER_LIMIT_BYTES = 2 * 1024 * 1024;
export const PHOTO_TARGET_BYTES = Math.floor(1.8 * 1024 * 1024);
...
export async function compressPhotoForUpload(
  ...
  if (file.size <= PHOTO_TARGET_BYTES && file.type === "image/jpeg") {
    return file;
  }
  ...
  if (bestBlob && bestBlob.size <= PHOTO_SERVER_LIMIT_BYTES) {
...
export function getPhotoUploadErrorMessage(
  ...
  if (error instanceof PhotoCompressionError) {
    if (error.reason === "compress_failed") {
      return t(`${keyPrefix}.photoCompressionFailed`);
    }
    return t(`${keyPrefix}.photoTooLargeAfterCompression`);
  }

  const uploadError = error as (Error & { status?: number }) | null;
  if (uploadError?.status === 413) {
    return t(`${keyPrefix}.photoTooLargeLimit`, { size: "2 MB" });
  }

  return error instanceof Error ? error.message : t(`${keyPrefix}.photoUpload`);
}
