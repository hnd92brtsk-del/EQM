...
export default function SerialMapPage() {
  ...
  return (
    <div className="eqm-canvas-page space-y-4 text-slate-900" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { void handleDropImport(event); }}>
      ...
      <div className="grid gap-4 xl:grid-cols-[320px_minmax(0,1fr)_340px]">
        ...
        <Card ref={canvasShellRef} className="overflow-hidden border-slate-200 shadow-none">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div ref={addNodeMenuRef} className="relative">
                    ...
                    {isAddNodeMenuOpen ? <div className="absolute left-0 top-full z-20 mt-2 w-44 border border-[var(--eqm-ui-border-strong)] bg-[var(--eqm-ui-panel)] p-1 shadow-xl">{(["master", "slave", "sensor", "bus", "repeater", "gateway"] as const).map((kind) => <button key={kind} type="button" className="flex w-full items-center justify-between px-3 py-2 text-left text-sm text-[var(--eqm-ui-text)] transition hover:bg-[var(--eqm-ui-panel-alt)]" onClick={() => { addPresetNode(kind); setIsAddNodeMenuOpen(false); }}>{kind === "master" ? "Master" : kind === "slave" ? "Slave" : kind === "sensor" ? "Sensor" : kind === "bus" ? "Bus" : kind === "repeater" ? "Repeater" : "Gateway"}</button>)}</div> : null}
                  </div>
                  <Button
                    size="icon"
          ...
          <CardContent className="p-0">
            <div ref={wrapperRef} onWheel={handleWheel} onPointerDown={beginCanvasInteraction} className={cn("eqm-canvas-shell relative h-[calc(100vh-260px)] min-h-[680px] overflow-hidden", isFullscreen && "h-screen min-h-screen")}>
